...
window.toggleDestino = async (id, actual) => {
  const orden = ['barra','cocina','pizzas','ambos'];
  const next = orden[(orden.indexOf(actual) + 1) % 4];
...
async function init() {
  ...
  onValue(ref(db, 'config/local'), snap => {
    ...
    document.getElementById('local-pizzas-font-size').value = d.pizzasFontSize || 9;
    ...
    document.getElementById('local-pizzas-uppercase').value = String(d.pizzasUppercase === true);
    ...
    document.getElementById('local-pizzas-print-service-id').value = d.pizzasPrintServiceId || '';
  });
...
window.guardarLocal = async () => {
  await set(ref(db, 'config/local'), {
    ...
    pizzasFontSize: parseFloat(document.getElementById('local-pizzas-font-size').value) || 9,
    ...
    pizzasUppercase: document.getElementById('local-pizzas-uppercase').value === 'true',
    ...
    pizzasPrintServiceId: document.getElementById('local-pizzas-print-service-id').value.trim(),
...
window.cierreCajaRapido = async () => {
  ...
  const efectivo = tickets.filter(t => (t.pagoMetodo || '').toLowerCase() !== 'tarjeta').reduce((sum, t) => sum + Number(t.total || 0), 0);
