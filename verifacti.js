...
export async function siguienteNumero(serie) {
  ...
  const cfgSnap = await get(ref(db, 'config/verifacti'));
  const cfg = cfgSnap.val() || {};
  const prefix = cfg.numPrefijo || '';
  const digits = Number(cfg.numDigitos) || 0;
  return prefix + String(actual).padStart(digits, '0');
}
