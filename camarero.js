...
initEnviarLongPress();
initPedidosLongPress();

function initPedidosLongPress() {
  ...
  btnPedido.style.webkitTouchCallout = 'none';
  btnPedido.style.webkitUserSelect = 'none';
  btnPedido.style.userSelect = 'none';

  btnPedido.removeAttribute('onclick');

  btnPedido.addEventListener('mousedown', start);
  btnPedido.addEventListener('touchstart', start, { passive: true });
  ...
  btnPedido.addEventListener('mouseup', cancel);
  btnPedido.addEventListener('touchend', cancel);
  btnPedido.addEventListener('touchmove', cancel);
  btnPedido.addEventListener('mouseleave', cancel);
  btnPedido.addEventListener('contextmenu', (e) => e.preventDefault());
}
...
function crearMesaBtn(id, m) {
  ...
  const permitirseCerrar = ocupada || esTemporal;
    ...
    if (permitirseCerrar) {
      ...
      const start = (e) => {
        ...
        timer = setTimeout(() => {
          isLongPress = true;
          timer = null;
          window.cerrarMesaEspecifica(id, nombre);
        }, 800);
      };
      ...
      el.addEventListener('click', () => abrirMesa(id, nombre, ocupada));
    }
  });
}
...
function showCobrarModal(total, lineasImprimir) {
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="font-family:var(--mono);font-size:14px;display:flex;flex-direction:column;gap:12px">
      <div>Total a cobrar: <strong>${fmtEu(total)}</strong></div>
  ...
  btnTj.onclick = () => {
    ...
    inp.disabled = true;
    document.getElementById('cobrar-error').style.display = 'none';
    btnCobrar.textContent = 'Imprimir y Cobrar';
  };
