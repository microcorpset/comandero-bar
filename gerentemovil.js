...
window.cargarVentasBajoDemanda = async () => {
  ...
  try {
    ...
    const data = snap.val() || {};
    const tickets = Object.values(data).filter(t => t.ts >= dStart && t.ts <= dEnd);
...
function renderAuditoria(logs) {
  ...
  logs.sort((a,b) => b.ts - a.ts).forEach(l => {
    ...
    let actClass = "";
    if (String(l.accion).includes("login")) actClass = "login";
    else if (String(l.accion).includes("mesa_cerrada")) actClass = "mesa_cerrada";
    else if (String(l.accion).includes("articulo_eliminado")) actClass = "articulo_eliminado";
    else if (String(l.accion).includes("descuento_aplicado")) actClass = "descuento_aplicado";

    item.className = `timeline-item ${actClass}`;
    
    item.innerHTML = `
      <div class="timeline-header">
        <span class="timeline-time">${l.hora || '00:00:00'}</span>
        <span class="timeline-user">${l.camarero || 'Desconocido'}</span>
      </div>
      <div class="timeline-action">${l.accion}</div>
...
window.confirmarCierreCaja = async () => {
  ...
  try {
    ...
    const snapHistorial = await get(ref(db, "historial"));
    const tickets = Object.values(snapHistorial.val() || {}).filter(t => t.ts >= startTs && t.ts <= endTs);
